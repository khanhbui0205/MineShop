  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const pkgs: CoinPackage[] = pkgsRes.data.map((p: any) => ({
        ...
        price: p.price,
        coinAmount: p.baseCoins ?? p.coinAmount,
        bonusCoin: p.bonusCoins ?? p.bonusCoin ?? 0,
        baseCoins: p.baseCoins ?? p.coinAmount,
        bonusCoins: p.bonusCoins ?? p.bonusCoin ?? 0,
        promotionPercent: p.promotionPercent ?? 0,
        promotionType: p.promotionType ?? ((p.bonusCoins ?? p.bonusCoin ?? 0) > 0 ? 'bonus_coin' : 'none'),
        discountPercent: p.discountPercent ?? 0,
        originalPrice: p.originalPrice ?? p.price,
        finalPrice: p.finalPrice ?? p.price,
        promotionBadgeText: p.promotionBadgeText ?? '',
  ...
  const handleAddPackage = async (pkg: Omit<CoinPackage, 'id'>) => {
    try {
      const res = await api.post('/admin/packages', {
        ...
        bonusCoin: pkg.bonusCoins ?? pkg.bonusCoin,
        bonusCoins: pkg.bonusCoins ?? pkg.bonusCoin,
        promotionType: pkg.promotionType ?? 'none',
